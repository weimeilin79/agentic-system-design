...
- **Rubber-stamping:** the queue outgrows the reviewer, and approval becomes theatre. This is a design problem wearing the mask of a discipline problem, and the fix is never "review faster."
...
The trajectory of a maturing system is up this ladder, one action type at a time, as evidence accumulates. But watch the arithmetic, because it is the bottleneck the whole industry is walking into. If an agent can propose faster than a human can review, then human-in-the-loop makes the human the throughput limit and the safety theatre inevitable. The way out is not a faster reviewer. It is moving the reversible work off the queue entirely, so the human's attention lands only where it changes an outcome. This is the review-throughput problem, and it gets a chapter of its own (Chapter 27) because it is the shape of the next few years.
