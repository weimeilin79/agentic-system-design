...
**Externalize.** *Keep the working state outside the window, and carry a reference instead of the payload.* The message thread grows monotonically. Every tool result and intermediate finding piles up until it rots the window. So write that material to storage outside the context, a scratchpad, a file, a state object, and keep only a lightweight pointer in-band, pulling the full content back only when it's needed. Many coding assistants today do this explicitly on long sessions, writing file paths, discovered patterns, and decisions to external state rather than dragging them through every message. This is the most direct answer to bloat, and the foundation the others build on. *Targets distraction and bloat.*
...
- **TODOs resolved:** (1) Cacheable Prefix now states plainly this isn't Manus-specific: every frontier provider offers prefix caching, and ADK has first-class support (`static_instruction` primitive, `ContextCacheConfig`); closing payoff and Targets line restored. (2) Keep the Errors distinguishes the in-run trace (the pattern) from the cross-session `gotcha.md` cousin (Externalize applied to mistakes, flagged for the memory chapter).
