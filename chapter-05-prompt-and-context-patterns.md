...
>
> *What this section should do:* Assemble the running example out of the named patterns: Structured Prompt + Output Contract + Exemplars; Externalize into memory for investigation notes; Just-in-Time Retrieval for the matching runbook; Compaction as the investigation grows; Context Isolation for a deep log-analysis subagent; Cacheable Prefix for the 3 a.m. token bill; Recitation for the current hypothesis; Keep the Errors so a failed remediation isn't retried. Close on the poisoning guard: keep failed attempts, never let a wrong root-cause guess harden into fact.
