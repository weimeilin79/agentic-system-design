...
>
> *What this section should do:* Walk the running example through the chapter's three groups. The store: all four types at once (working = live incident; episodic = past incidents in a vector archival tier; semantic = ownership in a temporal knowledge graph; procedural = runbooks), tiered and scoped per service/team. The pipeline: on incident close, Extract root cause + fix, Consolidate against similar incidents, Reconcile changed ownership; on recall, Score and Retrieve weighted for recency; overnight, one dream job Reflects new runbooks and Prunes the decommissioned one. The axes: mostly agent-managed, but ownership reconciliation stays programmatic; heavy work on background timing. Close on episodic becoming semantic becoming procedural.
